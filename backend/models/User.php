...
//            exit;
   ...
   {
      $model = self::find()->select(["CONCAT(`first_name`,' ',`last_name`) as name"])->asArray()->where(['id' => $id])->one();
   ...
   public static function GetAllUsersByDepartmentIdIndex($department_id)
   {
      if (empty($department_id)) {
         return self::GetAllUsersIndex();
      }
      return self::find()->select(["CONCAT(`first_name`,' ',`last_name`) as name", 'id'])->where(['department_id' => $department_id])->indexBy('id')->column();
   }
   ...
   public static function GetUsersWhichManagerMyByDepartment($year)
   {
